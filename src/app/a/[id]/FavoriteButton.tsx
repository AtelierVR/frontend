...
const KEY_PREFIX = 'public.favorite.avatar.';
...
export default function FavoriteButton({ avatarSid }: { avatarSid: string }) {
    const Api = useApi();
    ...
    const isFavorited = primaryEntry !== null && primaryEntry.values.includes(avatarSid);
    ...
    const togglePrimary = async () => {
        ...
        const next: FavoriteEntry = {
            label: current.label,
            values: current.values.includes(avatarSid)
                ? current.values.filter(s => s !== avatarSid)
                : [...current.values, avatarSid],
        };
    ...
    const addToTable = async (key: string) => {
        ...
        if (!current.values.includes(avatarSid)) {
            const next: FavoriteEntry = { label: current.label, values: [...current.values, avatarSid] };
    ...
    const createNewGroup = async () => {
        ...
        await Api.setMyTable(key, { values: [avatarSid] }, 'application/json+favorite');
        await loadTables();
        setBusy(false);
    };
